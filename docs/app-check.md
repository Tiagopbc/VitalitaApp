...
- `trainer_invites` deixa qualquer autenticado ler (`get`) um convite `active` e nao expirado. E necessario: o aluno precisa ler o convite para aceitar, e antes do vinculo existir nao ha de quem ele seja dono. O que o aluno digita e o proprio ID do documento — 8 caracteres sobre alfabeto de 32 simbolos gerado por `crypto.getRandomValues`, com TTL de 7 dias, ~10^12 combinacoes —, entao resgatar exige conhecer o codigo, e `list` fica restrito ao personal dono. **Nem sempre foi assim, e o episodio importa para esta decisao.** Ate 29/08/2026 o `code` era um campo achado por consulta, e um unico `allow read` cobria `get` e `list`: como o motor de rules prova um `list` pelos filtros da consulta, `where('status','==','active')` somado a um limite de `expiresAt` a frente do relogio do servidor devolvia todos os convites ativos, com `code` e `trainerId`, para qualquer autenticado — que assim queimava o convite de outro aluno e criava vinculo espurio. Fechado em 30/08/2026 (PR #86), fazendo o codigo virar o ID do documento e separando `get` de `list`; ver "Por que `get` e `list` sao separados" em [security-rules.md](security-rules.md). **App Check nao teria sido o controle disso**: o buraco estava na regra e no modelo de dados, e so regra e modelo de dados o fecharam. Enforcement ligado nao teria mudado nada — o atacante era um usuario autenticado no app legitimo.
